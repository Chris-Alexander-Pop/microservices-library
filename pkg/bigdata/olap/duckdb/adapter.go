...
// New opens a DuckDB database. Use ":memory:" for in-memory.
...
func (a *Adapter) Query(ctx context.Context, query string, args ...interface{}) (*bigdata.Result, error) {
	...
	for rows.Next() {
		...
		for i, col := range cols {
			val := values[i]
			// DuckDB specific type handling could go here
			if b, ok := val.([]byte); ok {
				row[col] = string(b)
			} else {
				row[col] = val
...
// Exec executes a statement without returning rows (DDL/DML).
