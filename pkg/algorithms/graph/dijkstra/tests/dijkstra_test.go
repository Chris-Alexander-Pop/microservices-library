...
func TestShortestPath(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := dijkstra.ShortestPath(g, tt.start, tt.end)
			if got == nil {
				t.Fatalf("dijkstra.ShortestPath() returned nil, want path")
			}
			if got.Distance != tt.wantDist {
				t.Errorf("dijkstra.ShortestPath().Distance = %v, want %v", got.Distance, tt.wantDist)
			}
			if !reflect.DeepEqual(got.Path, tt.wantPath) {
				t.Errorf("dijkstra.ShortestPath().Path = %v, want %v", got.Path, tt.wantPath)
			}
		})
	}
}
